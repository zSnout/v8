...
import { gather, GatherInfo } from "../db/study/gather"
import { saveReview, select } from "../db/study/select"
...
export const Study = (({ db, main, dids }, pop) => {
  ...
  async function answer(grade: Grade) {
    ...
    const now = Date.now()
    await saveReview(db, main, dids, c, info!, now, now - shownAt, grade)
    batch(() => {
      refetch()
    ...
    return (
      <Response class={props.class} onClick={() => answer(props.rating)}>
        ...
        {(() => {
          ...
          console.log({
            cardDue: item.card.due,
            cardLastReview: item.card.last_review,
            logDue: item.log.due,
            logReview: item.log.review,
          })
  ...
  // TODO: function QuickAction(props: {
  //   icon: IconDefinition
  //   label: string
  //   onClick?: () => void
  //   shortcut?: string
  ...
  //             return
  //           }
  //           getData().card?.reviewForget(
  //             Date.now(),
  //             Math.max(0, Date.now() - getData().shownAt),
  //             mode,
  //           )
  //           refetch()
  //         }}
    ...
    return (
      <div class="flex w-full flex-1 flex-col items-center justify-center gap-8">
        ...
        <div class="mx-auto flex w-full max-w-md flex-col gap-2">
          {/* TODO: <Show when={scheduler.new.length}>
  ...
  // TODO: function NullMoreNewAvailable() {
  //   return (
  //     <p>
...
// TODO: async function selectForgetMode(owner: Owner | null) {
