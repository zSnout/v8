...
>(
  ...
  return createLoading(
    load,
    (props, { async, item: initialItem, title, subtitle, save }, pop) => {
      ...
      const [selectedId, setSelectedId] = createSignal(
        notNull(entries()[0]?.id, strings.needAtLeastOne as never),
      )
          ...
          <div class="flex flex-col gap-1">
            <Action
              ...
              onClick={async () => {
                ...
                const name = await pickName(strings.newFieldName)

                if (!name) {
                  return
                }

                const next = create(name)
              ...
              onClick={async () => {
                ...
                batch(() => {
                  ...
                  const fields = setEntries((x) => {
                    ...
                    if (idx == -1) return x
                    if (x.length <= 1) throw new Error(strings.needAtLeastOne)
                    x.splice(idx, 1)
                    return [...x]
                  })

                  setSelectedId(
                    notNull(fields[0]?.id, strings.needAtLeastOne as never),
                  )
                })
              }}
            />
